...
The one-argument form reads the source in `*default-psl-url*` through
`clojure.java.io/reader`: open/read failures throw `java.io.IOException`, and an
unsupported source throws `java.lang.IllegalArgumentException`. For `n` PSL
entries, `k` matching ancestors, and encoded domain length `b`, lookup is
O(b * (log n + k))."
